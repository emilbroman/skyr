...
up: image scoc-image web-image deps
	podman compose -f dev/podman-compose.yml up -d --force-recreate web api scs de-0 de-1 rte-0 rte-1 rte-2 re-0 re-1 ne plugin-std-random plugin-std-time plugin-std-artifact plugin-std-crypto plugin-std-dns plugin-std-container scoc-1 scoc-2 scoc-3
