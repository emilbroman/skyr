...
impl<S> Program<S> {
    ...
    pub fn path_exists_cached(&self, resolved: &str) -> Option<bool> {
        ...
        for (i, component) in components.iter().enumerate() {
            ...
            let is_dir = children.iter().any(|entry| {
                matches!(entry, ChildEntry::Directory(name) if name == component)
            });
            if !is_dir {
...
impl<S: SourceRepo> Program<S> {
    ...
    pub async fn resolve_paths(&mut self) -> Result<Diagnosed<()>, ResolveImportError> {
        ...
        for (module_id, path_expr) in &collected {
            let resolved =
                path_expr.resolve_with_context(module_id, self.self_package_id.as_ref());
