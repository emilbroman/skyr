...
///   evaluation to the defining module's package so that manipulating a
///   `Path` value via `Std/Path.{join,parent,fromStr}` can resolve the
///   resulting path against the same package and preserve content-addressing.
/// - `hash` — the git object id of the entity at `path` within `package`.
///   `ObjId::null()` when no package map is registered (e.g. compile-time
