			...
			{#each env.deployments as deployment}
				...
				>
					<div class="flex items-center justify-between gap-2">
						<span class="text-sm text-white truncate">{deployment.commit.message.split('\n')[0]}</span>
						<span class="text-xs text-gray-500 font-mono shrink-0">{deployment.commit.hash.substring(0, 7)}</span>
					</div>
					<div class="mt-1.5">
