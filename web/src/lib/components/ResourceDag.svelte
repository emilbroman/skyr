<script lang="ts">
	...
	let dag = $derived.by(() => {
		...
		layer0.forEach((n, i) => orderMap.set(n.id, i));
		...
		for (let l = 1; l <= maxLayer; l++) {
			...
			layerNodes.forEach((n, i) => orderMap.set(n.id, i));
		}
	...
	function typeParts(type: string): { prefix: string; last: string } {
		...
		if (parts.length > 1) {
			return { prefix: parts.slice(0, -1).join('.') + '.', last: parts[parts.length - 1] };
		}
							...
							{#if node.href}
								<a
									...
									class="flex flex-col justify-center w-full h-full rounded-lg border px-3 py-2 transition-all duration-150
										...
										{hoveredNode === node.id ? (node.isShadow ? 'border-amber-500 shadow-lg shadow-amber-500/10' : 'border-indigo-500 shadow-lg shadow-indigo-500/10') : ''}
										{hoveredNode && hoveredNode !== node.id ? 'opacity-40' : ''}"
									onmouseenter={() => (hoveredNode = node.id)}
									onmouseleave={() => (hoveredNode = null)}
								>
									<div
										class="text-[10px] truncate {node.isShadow ? 'text-gray-500' : 'text-indigo-400/70'}"
									>
										{#if tp.prefix}<span>{tp.prefix}</span>{/if}
										<span class={node.isShadow ? 'text-gray-400' : 'text-indigo-300'}
											>{tp.last}</span
										>
									</div>
									<div class="flex items-center gap-1.5 mt-0.5">
										<span
											class="text-xs truncate {node.isShadow ? 'text-gray-500' : 'text-gray-300'}"
											>{node.name}</span
										>
										{#each node.markers as marker}
											<span
												class="text-[9px] px-1 py-px rounded border shrink-0 {marker === ResourceMarker.Volatile ? 'border-yellow-700 text-yellow-400' : 'border-blue-700 text-blue-400'}"
											>
												{marker}
											</span>
										{/each}
									...
									{#if node.isShadow}
										<div class="text-[9px] text-amber-500/70 mt-0.5">External</div>
									{/if}
								</a>
							{:else}
								<!-- svelte-ignore a11y_no_static_element_interactions -->
								<div
									class="flex flex-col justify-center w-full h-full rounded-lg border border-dashed px-3 py-2 transition-all duration-150
										...
										{hoveredNode === node.id ? 'border-amber-500 shadow-lg shadow-amber-500/10' : ''}
										{hoveredNode && hoveredNode !== node.id ? 'opacity-40' : ''}"
									onmouseenter={() => (hoveredNode = node.id)}
									onmouseleave={() => (hoveredNode = null)}
								>
									...
									<div class="flex items-center gap-1.5 mt-0.5">
										<span class="text-xs text-gray-500 truncate">{node.name}</span>
									</div>
									<div class="text-[9px] text-amber-500/70 mt-0.5">External</div>
								</div>
							{/if}
