<script lang="ts">
	...
	const styles: Record<DeploymentState, { bg: string; text: string }> = {
		...
		[DeploymentState.Desired]: { bg: 'bg-blue-900/40 border-blue-700', text: 'text-blue-300' },
		[DeploymentState.Down]: { bg: 'bg-gray-800 border-gray-600', text: 'text-gray-400' },
		[DeploymentState.Undesired]: { bg: 'bg-yellow-900/40 border-yellow-700', text: 'text-yellow-300' },
		[DeploymentState.Lingering]: { bg: 'bg-orange-900/40 border-orange-700', text: 'text-orange-300' }
	};
