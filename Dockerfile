...
RUN set -eu; \
    mkdir -p crates/cdb/src crates/de/src crates/rdb/src crates/rte/src crates/rtq/src crates/scl/src crates/sclc/src crates/scs/src; \
...
COPY crates ./crates
RUN set -eu; \
    cargo build --release -p scs -p de -p rte; \
    mkdir -p /artifacts; \
    for bin in scs de rte; do \
